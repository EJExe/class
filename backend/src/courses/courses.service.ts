...
export class CoursesService {
  ...
  async joinByInviteCode(userId: string, inviteCode: string) {
    ...
    if (!course) {
      throw new NotFoundException('Курс с указанным кодом не найден');
    }
    ...
    try {
      ...
      throw new ConflictException('Вы уже состоите в этом курсе');
    }
  ...
  async getRoles(userId: string, courseId: string) {
    ...
    return [
      ...(isAdmin ? [{ value: CourseRole.admin, label: '�������������' }] : []),
      { value: CourseRole.teacher, label: '�������������' },
      { value: CourseRole.assistant, label: '���������' },
      { value: CourseRole.student, label: '�������' },
    ];
  }
